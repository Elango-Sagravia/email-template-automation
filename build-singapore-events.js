/**
 * build-singapore-events.js
 * -------------------------
 * DOCX -> Mammoth HTML -> structured extraction -> MJML injection -> HTML
 *
 * Usage:
 *   node build-singapore-events.js "docx/singapore-events/2026/apr/10.docx"
 *
 * Expected layout tokens inside mjml-template/singapore-events/layout.mjml:
 *   {{%PREVIEW_TEXT%}}
...
const NEWSLETTER_SLUG = "singapore-events";
const TEMPLATE_DIR = path.join(ROOT, "mjml-template", NEWSLETTER_SLUG);
const LAYOUT_PATH = path.join(TEMPLATE_DIR, "layout.mjml");

const DEFAULT_IMAGE_SRC =
  "https://www.singaporesummary.com/email/images/REPLACE_ME.jpg";
const DEFAULT_IMAGE_ALT = "REPLACE_ME";
const DEFAULT_IMAGE_HREF = "https://www.singaporesummary.com/";
const ACCENT_COLOR = "#660000";
...
async function main() {
  if (!DOCX_PATH) {
    throw new Error(
      'Usage: node build-singapore-events.js "docx/singapore-events/2026/apr/10.docx"',
    );
  }
  ...
  finalMjml = finalMjml.replace(
    ...
    renderFeatureCard(data.spotlight),
  );
  ...
  finalMjml = finalMjml.replace(
    ...
    renderFeatureCard(data.featured),
  );
...
function finalizeStory(story) {
  ...
  function parseTextLine(txt, href) {
    ...
    // plain linked lines stay in body, so they render exactly as doc content
    if (href) {
      body.push({
        text: txt,
        href,
...
function findLastChunkWithHref(chunks) {
  for (let i = chunks.length - 1; i >= 0; i--) {
    const chunk = chunks[i];
    const href = chunk.find("a[href]").first().attr("href") || "";
    if (href) {
      return { href };
    }
  }
...
<tr>
  ...
  <td
    style="
      font-size: 16px;
      ${last ? "line-height: 1.6;" : "line-height: 24px; padding-bottom: 11px;"}
    "
  >
    ${escapeHtml(item)}
  </td>
...
function renderFeatureCard(story) {
...
function renderListItem(item, addDivider) {
  const descHtml = (item.body || [])
    .map((p) => {
      if (typeof p === "string") {
        return `<p style="line-height: 24px">${escapeHtml(p)}</p>`;
      }

      if (p && typeof p === "object" && p.href && p.text) {
